...
class VideoItem extends Component {
    constructor(props) {
        super(props);
        this.state = {shouldShowVideo : true,
                      errorString: ''};
    }
    ...
    render() {
        ...
        if (this.state.shouldShowVideo) {
            VideoPlayer = (<ReactPlayer className="react-player"
                                  url={item.url}
                                  key={item.videoId}
                                  width='100%'
                                  height='100%'
                                  onError={() => this.onVideoError(item.url)}
                                  playing muted playsinline/>);
        } else {
            // On Video Load Error
            VideoPlayer = (<div className="video-load-error-text">{ERR_LOADING_VIDEO_PREFIX} : {this.state.errorString}</div>);
        }
